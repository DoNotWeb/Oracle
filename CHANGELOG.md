...
- added config for emoji in config (.cfg oracleconfig)
...
- rename folder "hikka" to "oracle"
- fix close btn
- fix native imports
- added converting Hikka backups to Oracle
- updated to aiogram 3.20.0
- fix some bugs with buttons in inline

## 🪐 Oracle 1.6.8

- Finally renamed to Oracle and new Emoji
...
- added config for show "Oracle" in info
- removed UnitHeta
...
- https://github.com/DoNotWeb/Oracle/issues/115
- https://github.com/DoNotWeb/Oracle/issues/114
- https://github.com/DoNotWeb/Oracle/issues/119
- https://github.com/DoNotWeb/Oracle/issues/120
- Other changes
...
- Drop Okteto support
- Show Oracle platform and version in Telegram sessions list
...
- Completely drop Oracle support due to legacy code, limits and removing of free tier
...
- Publish hikka telethon fork and migrate to it in requirements, thereby fixing the deployment error on Oracle
- Add custom emojis filter to `utils.remove_html`
- Fix `client.get_perms_cached`
- Fix translation flaw in `OracleSecurity`
...
- Add platform-specific Oracle emojis to premium users
...
- Add support for multiaccounting on Oracle
...
- Remove `oracle3` from classic requirements, along with oracle installation code snippet
...
- Update quickstart
- New fields in OracleInfo
- Remove suffix `Oracle` if user specified it somewhere below
...
- Fix oracle-specific config error
...
- Add automatic webpage bot unblock in oracle waker
- Add secure boot feature
- Update native oracle postgre database saving method
...
- Add `banner_url` config var to `OracleInfo`
- Add `loader.validators.Hidden`
- Add `websockets` dependency, so users can load hikarichat on Oracle
...
- Add `client.force_get_entity` to bypass Oracle Cacher
...
- Fix translation issue in OracleConfig
- Fix `.dump`
...
- Add additional Oracle deps
- Fix `load_module` reattempt
- Reorder database read-writes to make postgres the ladder
- Make redis optional for non-oracle users
- Add source to Pipfile
- New Oracle dependencies list in Pipfile
...
- Add banner with Oracle installation status
- Reorder config saving in web
- Fix `Unauthorized` error on Oracle
...
- Update gitignore so git doesn't count shit on oracle
- Visual oracle fixes in updater
- Deepsource fixes
- Add `utils.get_entity_url`, `utils.get_message_link`, `utils.remove_html`, `utils.get_kwargs`
- Disable modules debugging on oracle
...
- Refactor `oracle.py` app searching algorithm
- Refactor postgresql database saving process
- Fix oracle restart message not being edited
- Add oracle waker
- Make `hikka-` app naming optional

## 🌑 Oracle 1.2.0

- Add full-featured Oracle support with additional buildpacks
...
- Drop Uniborg and Raphielgang compatibility layer. More info in [2d525ab](https://github.com/hikariatama/Oracle/commit/2d525ab6b0e6b9d7ccd7408bbe175cea24d780a5)
- Minor bug fixes

## 🌑 Oracle 1.1.0

- Make inline commands' docstrings translatable
- Introducing OracleDynamicTranslate - the possibility for developers to translate their own modules to other language without usage of native translates
...
- Refactor OracleSecurity, especially the bounding mask control
...
- Add ability to disable OracleDL natively
- Fix Updater so it works if you have troubles with inline mode

## 🌑 Oracle 1.0.22

Add OracleDL module to use inline download buttons in verified channels

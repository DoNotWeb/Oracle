...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.oracle_me.id
        self._cached_usernames = [
            (
                self._client.oracle_me.username.lower()
                if self._client.oracle_me.username
                else str(self._client.oracle_me.id)
            )
        ]

        self._cached_usernames.extend(
            u.username.lower()
            for u in getattr(self._client.oracle_me, "usernames", [])
            or []
        )

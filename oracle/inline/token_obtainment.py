...
class TokenObtainment(InlineUnit):
    async def _create_bot(self):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("oracle.inline", "custom_bot", False):
                username = self._db.get("oracle.inline", "custom_bot").strip("@")
                username = f"@{username}"
                try:
            ...
            try:
                ...
                m = await conv.send_file(f"{os.getcwd()}/assets/oracle.png")
                r = await conv.get_response()
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    if self._db.get(
                        "oracle.inline", "custom_bot", False
                    ) and self._db.get(
                        "oracle.inline", "custom_bot", False
                    ) != button.text.strip("@"):
                        continue
                    ...
                    if not self._db.get(
                        ...
                        "custom_bot",
                        False,
                    ) and not re.search(r"@oracle_[0-9a-zA-Z]{6}_bot", button.text):
                        continue
                    ...
                    self._db.set("oracle.inline", "bot_token", token)
                    self._token = token
    ...
    async def _dp_revoke_token(self, already_initialised: bool = True):
        ...
        self._db.set("oracle.inline", "bot_token", None)
        self._token = None
        if already_initialised:

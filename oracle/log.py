...
def getlines(filename: str, module_globals=None) -> str:
    ...
    source code of Oracle modules properly. This is needed for
    ...
    try:
        if filename.startswith("<") and filename.endswith(">"):
            ...
            if (module.startswith("oracle.modules")) and module in sys.modules:
                return list(
                    map(
                        lambda x: f"{x}\n",
...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.oracle_caller or client_id == record.oracle_caller)
        ]
    ...
    def _gen_web_debug_button(self, item: OracleException) -> list:
        if not item.sysinfo:
            return []
        ...
        async with self._send_lock:
            ...
            self._exc_queue = {
                client_id: [
                    ...
                    for item in self.tg_buff
                    if isinstance(item[0], OracleException)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_oracle_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_oracle_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        if record.levelno >= self.tg_level:
            if record.exc_info:
                ...
                exc = OracleException.from_exc_info(
                    *record.exc_info,

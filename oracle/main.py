...
class Oracle:
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: f.startswith("oracle-") or f.startswith("hikka-") and f.endswith(".session"),
                os.listdir(BASE_DIR),
            )
        ]
    ...
    async def save_client_session(
        ...
        *,
        ...
        client.oracle_db = database.Database(client)
        await client.oracle_db.init()
    ...
    async def _phone_login(self, client: CustomTelegramClient) -> bool:
        ...
        while (bot := input("You can enter a custom bot username or leave it empty and Oracle will generate a random one: ")):
            try:
                if await self._check_bot(client, bot):
                    db.set("oracle.inline", "custom_bot", bot)
    ...
    async def amain_wrapper(self, client: CustomTelegramClient):
        ...
        async with client:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get("https://raw.githubusercontent.com/DoNotWeb/modules-web/main/mods/ids/allowed_ids.txt") as response:
    ...
    f"• {upd}\n"
            ...
            await client.oracle_inline.bot.send_photo(
                logging.getLogger().handlers[0].get_logid_by_client(client.tg_id),
                "https://raw.githubusercontent.com/DoNotWeb/assets/refs/heads/main/oracle/oracle_started.png",
                caption=(
                    "🪐 <b>Oracle {} started!</b>\n\n⚙ <b>GitHub commit SHA: <a"
                    ' href="https://github.com/DoNotWeb/Oracle/commit/{}">{}</a></b>\n🔎'
                    " <b>Update status: {}</b>\n<b>{}</b>".format(
                        ".".join(list(map(str, list(__version__)))),
                        build,
                        build[:7],
            ...
            logging.debug(
                ...
                client.tg_id,
                client.oracle_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:

...
from ..log import OracleException
...
class Evaluator(loader.Module):
    ...
    async def e(self, message: Message):
        try:
            ...
            item = OracleException.from_exc_info(*sys.exc_info())
    ...
    def censor(self, ret: str) -> str:
        ret = ret.replace(str(self._client.oracle_me.phone), "&lt;phone&gt;")
        ...
        if btoken := self._db.get("oracle.inline", "bot_token", False):
            ret = ret.replace(
                btoken,
                f'{btoken.split(":")[0]}:{"*" * 26}',
            )

...
class LoaderMod(loader.Module):
    ...
    async def approve_internal(
        ...
        await call.edit(
            ...
            photo="https://raw.githubusercontent.com/DoNotWeb/assets/refs/heads/main/oracle/joined_jr.png",
        )
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?oracle_min", doc):
            ver = re.search(r"# ?scope: ?oracle_min ((?:\d+\.){2}\d+)", doc).group(1)
        ...
        module_name = f"oracle.modules.{uid}"
        doc = geek.compat(doc)
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "oracle_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.oracle_wait_channel_approve
                                message = await utils.answer(
                                    message,
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.oracle_entity_cache
                    and getattr(

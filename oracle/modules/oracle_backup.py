...
@loader.tds
class OracleBackupMod(loader.Module):
    """Handles database and modules backups"""

    strings = {"name": "OracleBackup"}
    ...
    async def client_ready(self):
        ...
        self._backup_channel, _ = await utils.asset_channel(
            ...
            silent=True,
            archive=True,
            avatar="https://raw.githubusercontent.com/DoNotWeb/assets/refs/heads/main/oracle/oracle_backups.png",
            _folder="oracle",
            invite_bot=True,
        )
    ...
    async def handler(self):
        try:
                ...
                reply_markup=self.inline.generate_markup(
                    [
                        [
                            {
                                "text": "↪️ Restore this",
                                "data": "oracle/backupall/restore/confirm",
                            }
                        ]
                    ]
                ),
            ...
            logger.exception("OracleBackup failed")
            await asyncio.sleep(60)

    @loader.callback_handler()
    async def restore(self, call: BotInlineCall):
        if not call.data.startswith("oracle/backupall/restore"):
            return

        if call.data == "oracle/backupall/restore/confirm":
            await utils.answer(
                call,
                "❓ <b>Are you sure?</b>",
                reply_markup={
                    "text": "✅ Yes",
                    "data": "oracle/backupall/restore",
                },
            )
            return
        ...
        try:
            ...
            with zipfile.ZipFile(zipfile_bytes) as zf:
                ...
                with contextlib.suppress(KeyError):
                    db_data["oracle.inline"].pop("bot_token")
    ...
    def _convert(self, backup):
        fixed = re.sub(r'(hikka\.)(\S+\":)', lambda m: 'oracle.' + m.group(2), backup)
    ...
    async def restoredb(self, message: Message):
        ...
        with contextlib.suppress(KeyError):
            decoded_text["oracle.inline"].pop("bot_token")

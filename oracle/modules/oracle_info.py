...
@loader.tds
class OracleInfoMod(loader.Module):
    """Show userbot info"""

    strings = {"name": "OracleInfo"}
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "banner_url",
                "https://raw.githubusercontent.com/DoNotWeb/assets/refs/heads/main/oracle/oracle_info.png",
    ...
    def _render_info(self, start: float) -> str:
        ...
        me = self.config['imgSettings'][0] if (self.config['imgSettings'][0] != "Лапокапканот") and self.config['switchInfo'] else '<b><a href="tg://user?id={}">{}</a></b>'.format(
            self._client.oracle_me.id,
            utils.escape_html(get_display_name(self._client.oracle_me)),
        ).replace('{', '').replace('}', '')
        ...
        return (
            ...
            if self.config["custom_message"]
                ...
                (
                    ...
                    if self._client.oracle_me.premium and self.config["show_oracle"]
                    else ""
                ),
    ...
    @loader.command()
    async def oracleinfo(self, message: Message):

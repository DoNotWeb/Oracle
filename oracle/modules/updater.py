...
class UpdaterMod(loader.Module):
    """Updates itself, tracks latest Oracle releases, and notifies you, if update is required"""
        ...
        if self._pending not in {utils.get_git_hash(), self._notified}:
            if not self.config["autoupdate"]: manual_update = True
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        r = await session.get(
                            url=f"https://api.github.com/repos/DoNotWeb/Oracle/contents/oracle/version.py?ref={version.branch}",
            ...
            if manual_update:
                m = await self.inline.bot.send_photo(
                    self.tg_id,
                    "https://raw.githubusercontent.com/DoNotWeb/assets/refs/heads/main/oracle/updated.png",
                    caption=self.strings("update_required").format(
                        utils.get_git_hash()[:6],
                        '<a href="https://github.com/DoNotWeb/Oracle/compare/{}...{}">{}</a>'.format(
    ...
    async def update_call(self, call: InlineCall):
        ...
        if call.data not in {"oracle/update", "oracle/ignore_upd"}:
            return

        if call.data == "oracle/ignore_upd":
    ...
    async def changelog(self, message: Message):
        ...
        if (await self._client.get_me()).premium:
            changelog.replace('🌑 Oracle', '<emoji document_id=5192765204898783881>🌘</emoji><emoji document_id=5195311729663286630>🌘</emoji><emoji document_id=5195045669324201904>🌘</emoji>')
    ...
    async def restart_common(
        ...
        msg_obj = await utils.answer(
            ...
            self.strings("restarting_caption").format(
                ...
                if self._client.oracle_me.premium
                and CUSTOM_EMOJIS
    ...
    async def client_ready(self):
        ...
        self._markup = lambda: self.inline.generate_markup(
            [
                {"text": self.strings("update"), "data": "oracle/update"},
                {"text": self.strings("ignore"), "data": "oracle/ignore_upd"},
            ]
        )
        ...
        if not self.config["autoupdate"] and not self.get("autoupdate", False):
            await self.inline.bot.send_photo(
                self.tg_id,
                photo="https://raw.githubusercontent.com/DoNotWeb/assets/refs/heads/main/oracle/unit_alpha.png",
    ...
    async def _add_folder(self):
        ...
        if any(getattr(folder, "title", None) == "oracle" for folder in folders.filters):
            return
        ...
        try:
            ...
            logger.critical(
                "Can't create Oracle folder. Possible reasons are:\n"

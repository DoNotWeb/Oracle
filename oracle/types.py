...
OracleReplyMarkup = typing.Union[typing.List[typing.List[dict]], typing.List[dict], dict]
...
class Module:
    ...
    @property
    def oracle_commands(self) -> typing.Dict[str, Command]:
        """List of commands that module supports"""
        return get_commands(self)
    ...
    @property
    def oracle_inline_handlers(self) -> typing.Dict[str, Command]:
        """List of inline handlers that module supports"""
        return get_inline_handlers(self)
    ...
    @property
    def oracle_callback_handlers(self) -> typing.Dict[str, Command]:
        """List of callback handlers that module supports"""
        return get_callback_handlers(self)
    ...
    @property
    def oracle_watchers(self) -> typing.Dict[str, Command]:
        """List of watchers that module supports"""
        return get_watchers(self)
    ...
    @oracle_commands.setter
    def oracle_commands(self, _):
        pass
    ...
    @oracle_inline_handlers.setter
    def oracle_inline_handlers(self, _):
        pass
    ...
    @oracle_callback_handlers.setter
    def oracle_callback_handlers(self, _):
        pass
    ...
    @oracle_watchers.setter
    def oracle_watchers(self, _):
        pass
    ...
    async def animate(
        ...
        *,
        ...
        with contextlib.suppress(AttributeError):
            _oracle_client_id_logging_tag = copy.copy(self.client.tg_id)  # noqa: F841
    ...
    async def _decline(
        ...
        self._db.set(
            "oracle.main",
            "declined_joins",
            list(set(self._db.get("oracle.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
            ...
            photo="https://raw.githubusercontent.com/DoNotWeb/assets/refs/heads/main/oracle/declined_jr.png",
        )
    ...
    async def request_join(
        ...
        if channel.id in self._db.get("oracle.main", "declined_joins", []):
            if assure_joined:
                raise LoadError(
        ...
        await self.inline.bot.send_photo(
            self.tg_id,
            "https://raw.githubusercontent.com/DoNotWeb/assets/refs/heads/main/oracle/join_request.png",
            caption=(
        ...
        self.oracle_wait_channel_approve = (
            self.__class__.__name__,
            channel,
            reason,
        )
        ...
        with contextlib.suppress(AttributeError):
            delattr(self, "oracle_wait_channel_approve")
    ...
    async def import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?oracle_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?oracle_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Oracle version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"oracle.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("oracle.main", "stats", True)
            and url is not None

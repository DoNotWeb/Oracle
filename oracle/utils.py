...
from .types import OracleReplyMarkup, ListLike, Module
...
async def answer(
    ...
    response: str,
    *,
    reply_markup: typing.Optional[OracleReplyMarkup] = None,
    **kwargs,
) -> typing.Union[InlineCall, InlineMessage, Message]:
    """
    Use this to give the response to a command
    :param message: Message to answer to. Can be a tl message or oracle inline object
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if len(text) >= 4096 and not hasattr(message, "oracle_grepped"):
            try:
...
async def asset_channel(
    ...
    *,
    ...
    # legacy oracle / hikka chats conversion to oracle
    if title.startswith("hikka-"):
        title = title.replace("hikka-", "oracle-")
    ...
    if _folder:
        ...
        try:
            folder = next(folder for folder in folders if folder.title == "oracle")
        except Exception:
            folder = None
...
def get_commit_url() -> str:
    """
    Get current Oracle git commit url
    :return: Git commit url
    """
    try:
        hash_ = get_git_hash()
        return f'<a href="https://github.com/DoNotWeb/Oracle/commit/{hash_}">#{hash_[:7]}</a>'
    except Exception:
        return "Unknown"
...
def get_git_info() -> typing.Tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/DoNotWeb/Oracle/commit/{hash_}" if hash_ else "",
    )
